...
void set_pt_entry(pt_entry_t* ptentry, uint32_t user, uint32_t offset) {
    ...
    ptentry->address_31_12 = offset; // Physical address of video memory (0xB8000 >> 12)
}
...
void setup_kernel_paging() {
    ...
    set_pt_entry(&video_memory_page, 0, 0xB8);
    ...
    set_pt_entry(&video_memory_page1, 0, 0xB8 + 1);
    pt0[0xB8 + 1] = video_memory_page1.val;

    pt_entry_t video_memory_page2;
    set_pt_entry(&video_memory_page2, 0, 0xB8 + 2);
    pt0[0xB8 + 2] = video_memory_page2.val;

    pt_entry_t video_memory_page3;
    set_pt_entry(&video_memory_page3, 0, 0xB8 + 3);
    pt0[0xB8 + 3] = video_memory_page3.val;

    // At 128kb
    pt_entry_t audio_memory_page;
    set_pt_entry(&audio_memory_page, 0, 32);
    pt0[32] = audio_memory_page.val;
