...
// Global definition of file operation function pointers for RTC (Real-Time Clock) files.
FileOperationsTable dsp_operations_table = {
    .read = dsp_read,
    .write = dsp_write,
    .open = dsp_open,
    .close = dsp_close
...
int32_t execute(const uint8_t* command_user) {
    ...
    new_PCB->sig_data.sig_mask = 0;
    new_PCB->sig_data.signals_active[0] = 0;
    new_PCB->sig_data.signals_active[1] = 0;
    new_PCB->sig_data.signals_active[2] = 0;
    new_PCB->sig_data.signals_active[3] = 0;
    new_PCB->sig_data.signals_active[4] = 0;
...
int32_t open(const uint8_t* filename) {
    ...
    dir_entry_t dentry;
    if (read_dentry_by_name(filename, &dentry) == -1 && strncmp((int8_t*)filename, (int8_t*)"dsp", 3) != 0) // get dentry
        RETURN(-1);
    // get the file type
    uint32_t file_type = dentry.file_type;
    // Update attributes of the file desriptors according to type
    if (strncmp((int8_t*)filename, (int8_t*)"dsp", 3) == 0) {
        current_pcb->files[i].operationsTable = dsp_operations_table;
        current_pcb->files[i].filePosition = 0;
        current_pcb->files[i].flags = 1;
        RETURN(i); // Return FD number
    } else if ( file_type == 0) { // RTC file
...
int32_t set_handler(int32_t signum, void* handler_address) {
    ...
    asm volatile (
        ...
        "andl $0xFFFFE000, %%eax\n"  // Clear the lower 13 bits to align to 8KB boundary
        "movl %%eax, %0\n"           // Move the modified EAX value to current_pcb
        : "=r" (current_pcb)         // Output operand: current_pcb is written with the value from EAX
        :
        : "eax"                      // Clobber list, indicating EAX is modified
    );

    current_pcb->sig_data.handler_functions[signum] = (uint32_t*)handler_address;
...
int32_t sigreturn(void) {
    ...
    uint32_t user_esp = (uint32_t)current_pcb->sig_data.hardware_ctx.iret_esp;
    // Pop return address from stack (dont save)
    uint32_t dummy;
    POP_LONG(user_esp, dummy);

    // pop return EPB from stack (save for later)
    uint32_t sys_call;
    uint32_t eax;
    uint32_t ebx;
    uint32_t ecx;
    ...
    POP_LONG(user_esp, ebx);
    POP_LONG(user_esp, ecx);
    POP_LONG(user_esp, edx);
    POP_LONG(user_esp, esi);
    POP_LONG(user_esp, edi);
    POP_LONG(user_esp, ebp);
    POP_LONG(user_esp, eax);
    POP_LONG(user_esp, sys_call);
    POP_LONG(user_esp, iret_ret);
    POP_LONG(user_esp, iret_cs);
    POP_LONG(user_esp, iret_eflags);
    POP_LONG(user_esp, iret_esp);
    POP_LONG(user_esp, iret_ss);

    // pop linkage to sysreturn (don't save)
    POP_LONG(user_esp, dummy);
    POP_LONG(user_esp, dummy);

    current_pcb->sig_data.sig_mask = 0;

    // Overright restore context
    MOVE_VAR_TO_EBP_OFFSET(84, iret_ss);
    MOVE_VAR_TO_EBP_OFFSET(80, iret_esp);
    MOVE_VAR_TO_EBP_OFFSET(76, iret_eflags);
    MOVE_VAR_TO_EBP_OFFSET(72, iret_cs);
    MOVE_VAR_TO_EBP_OFFSET(68, iret_ret);

    MOVE_VAR_TO_EBP_OFFSET(60, ebx);
    MOVE_VAR_TO_EBP_OFFSET(56, ecx);        
    MOVE_VAR_TO_EBP_OFFSET(52, edx);
    MOVE_VAR_TO_EBP_OFFSET(48, esi);         
    MOVE_VAR_TO_EBP_OFFSET(44, edi);        
    MOVE_VAR_TO_EBP_OFFSET(40, ebp);

    if(!sys_call) {
        asm ("movl %0, %%eax" : : "r" (eax));
    }
...
void update_user_esp_syscall() {
    ...
    MOVE_FROM_EBP_OFFSET_TO_VAR(84, current_pcb->sig_data.hardware_ctx.iret_ss);
    MOVE_FROM_EBP_OFFSET_TO_VAR(80, current_pcb->sig_data.hardware_ctx.iret_esp);
    MOVE_FROM_EBP_OFFSET_TO_VAR(76, current_pcb->sig_data.hardware_ctx.iret_eflags);
    MOVE_FROM_EBP_OFFSET_TO_VAR(72, current_pcb->sig_data.hardware_ctx.iret_cs);
    MOVE_FROM_EBP_OFFSET_TO_VAR(68, current_pcb->sig_data.hardware_ctx.iret_ret);

    MOVE_FROM_EBP_OFFSET_TO_VAR(60, current_pcb->sig_data.hardware_ctx.ebx);
    MOVE_FROM_EBP_OFFSET_TO_VAR(56, current_pcb->sig_data.hardware_ctx.ecx);        
    MOVE_FROM_EBP_OFFSET_TO_VAR(52, current_pcb->sig_data.hardware_ctx.edx);
    MOVE_FROM_EBP_OFFSET_TO_VAR(48, current_pcb->sig_data.hardware_ctx.esi);         
    MOVE_FROM_EBP_OFFSET_TO_VAR(44, current_pcb->sig_data.hardware_ctx.edi);        
    MOVE_FROM_EBP_OFFSET_TO_VAR(40, current_pcb->sig_data.hardware_ctx.ebp);   

    current_pcb->sig_data.hardware_ctx.sys_call = 1;
...
void update_user_esp_exception(int vector) {
    ...
    if(current_pcb->sig_data.hardware_ctx.iret_esp > 0x8000000) {
        current_pcb->sig_data.prev_hardware_ctx = current_pcb->sig_data.hardware_ctx;
    }

    uint32_t offset = 0;
    if(vector == 0x8 || vector == 0xA || vector == 0xB || vector == 0xC  || vector == 0xD || vector == 0xE || vector == 0x11 || vector == 0x12 || vector == 0x13) {
        offset = 4;
    }

    MOVE_FROM_EBP_OFFSET_TO_VAR(704 + offset, current_pcb->sig_data.hardware_ctx.iret_ss);
    MOVE_FROM_EBP_OFFSET_TO_VAR(700 + offset, current_pcb->sig_data.hardware_ctx.iret_esp);
    MOVE_FROM_EBP_OFFSET_TO_VAR(696 + offset, current_pcb->sig_data.hardware_ctx.iret_eflags);
    MOVE_FROM_EBP_OFFSET_TO_VAR(692 + offset, current_pcb->sig_data.hardware_ctx.iret_cs);
    MOVE_FROM_EBP_OFFSET_TO_VAR(688 + offset, current_pcb->sig_data.hardware_ctx.iret_ret);

    MOVE_FROM_EBP_OFFSET_TO_VAR(680, current_pcb->sig_data.hardware_ctx.eax);
    MOVE_FROM_EBP_OFFSET_TO_VAR(676, current_pcb->sig_data.hardware_ctx.ecx);
    MOVE_FROM_EBP_OFFSET_TO_VAR(672, current_pcb->sig_data.hardware_ctx.edx);        
    MOVE_FROM_EBP_OFFSET_TO_VAR(668, current_pcb->sig_data.hardware_ctx.ebx);
    MOVE_FROM_EBP_OFFSET_TO_VAR(664, current_pcb->sig_data.hardware_ctx.ebp);         
    MOVE_FROM_EBP_OFFSET_TO_VAR(660, current_pcb->sig_data.hardware_ctx.esi);        
    MOVE_FROM_EBP_OFFSET_TO_VAR(656, current_pcb->sig_data.hardware_ctx.edi);   

    current_pcb->sig_data.hardware_ctx.sys_call = 0;
...
void sig_test() {
    ...
    if(current_pcb->sig_data.sig_mask == 1) {
        return;
    }
    ...
    int i;
    for(i = 0; i < 5; i++) {
        if(current_pcb->sig_data.signals_active[i] == 1) {
            sig_num = i;
            break;
        }
    }
    ...
    // Find handler for active signal
    uint32_t handler = (uint32_t)current_pcb->sig_data.handler_functions[sig_num];

    if(handler == 0) {
        // Default Action
        if(sig_num >= 0 && sig_num <= 2) {
            halt(256);
        }
    ...
    if(user_esp < 0x8000000) {
        current_pcb->sig_data.hardware_ctx = current_pcb->sig_data.prev_hardware_ctx;
        return;
    }

    current_pcb->sig_data.sig_mask = 1;
    current_pcb->sig_data.signals_active[sig_num] = 0;

    // Push linkage to sigreturn onto stack
    uint32_t linkage1 = 0x0080CD00;
    uint32_t linkage2 = 0x00000AB8;
    PUSH_LONG(user_esp, linkage1);
    PUSH_LONG(user_esp, linkage2);

    // Push current EBP onto stack
    PUSH_LONG(user_esp, current_pcb->sig_data.hardware_ctx.iret_ss);
    PUSH_LONG(user_esp, current_pcb->sig_data.hardware_ctx.iret_esp);
    PUSH_LONG(user_esp, current_pcb->sig_data.hardware_ctx.iret_eflags);
    PUSH_LONG(user_esp, current_pcb->sig_data.hardware_ctx.iret_cs);
    PUSH_LONG(user_esp, current_pcb->sig_data.hardware_ctx.iret_ret);
    PUSH_LONG(user_esp, current_pcb->sig_data.hardware_ctx.sys_call);
    PUSH_LONG(user_esp, current_pcb->sig_data.hardware_ctx.eax);
    PUSH_LONG(user_esp, current_pcb->sig_data.hardware_ctx.ebp);
    PUSH_LONG(user_esp, current_pcb->sig_data.hardware_ctx.edi);
    PUSH_LONG(user_esp, current_pcb->sig_data.hardware_ctx.esi);
    PUSH_LONG(user_esp, current_pcb->sig_data.hardware_ctx.edx);
    PUSH_LONG(user_esp, current_pcb->sig_data.hardware_ctx.ecx);
    PUSH_LONG(user_esp, current_pcb->sig_data.hardware_ctx.ebx);

    // Push single number
    PUSH_LONG(user_esp, sig_num);

    // Save return to system linkage on stack
    uint32_t return_address = user_esp + 56;
    PUSH_LONG(user_esp, return_address);

    // Set up context to handler with new top stack in user mode
    uint32_t ss = USER_DS;
    uint32_t esp = user_esp;
    uint32_t eflags = 0x00000202; // Allow interrupts
    uint32_t cs = USER_CS;
    uint32_t eip = handler;
    // Shifts bytes of the ESP from file approriate amount to form file EIP.
