...
void exc_handler(int vector) {
    update_user_esp_exception(vector); // Needs recalibration
    ...
    if(vector >= 0 && vector <= 0x13) { // 0x00 to 0x13: CPU exception vectors
        ...
        if(vector == 0) {
            current_pcb->sig_data.signals_active[0] = 1;
        } else {
            current_pcb->sig_data.signals_active[1] = 1;
        }
...
void setup_IDT() {
    ...
    SET_IDT_ENTRY(entry, DSP_linkage);
    set_IDT_entry_metadata(&entry, interrupt);
    idt[0x25] = entry; // 0x28 RTC interrupt vector
